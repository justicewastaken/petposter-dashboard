...
def fetch_daily_insights(ig_id, token, days=28):
    """
    Daily views and profile_views for the last N days.
    Returns a list of { date, views, profile_views } dicts sorted oldest->newest.
    """
    until = int(datetime.now(timezone.utc).timestamp())
    ...
    by_date = {}
    for item in data.get("data", []):
        name = item["name"]   # "views" or "profile_views"
        for v in item.get("values", []):
            date = v["end_time"][:10]
            if date not in by_date:
                by_date[date] = {"date": date, "views": 0, "profile_views": 0}
            by_date[date][name] = v["value"]
...
def build_account_data(acct):
    ...
    daily_insight = fetch_daily_insights(ig_id, token, days=28)
